...
async function startServer() {
  ...
  app.get("/api/users", authenticateToken, (req, res) => {
    const data = db.prepare("SELECT id, username FROM users").all();
    res.json(data);
  });
  ...
  app.put("/api/users/:id", authenticateToken, (req, res) => {
    ...
    try {
      if (password) {
        const hashedPassword = bcrypt.hashSync(password, 10);
        db.prepare("UPDATE users SET username = ?, password = ? WHERE id = ?").run(username, hashedPassword, req.params.id);
      } else {
  ...
  app.delete("/api/users/:id", authenticateToken, (req, res) => {
    try {
      ...
      // but for simplicity, just allow it for now.
